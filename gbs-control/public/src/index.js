...
const createWebSocket = () => {
    ...
    GBSControl.ws.onmessage = (message) => {
        ...
        if (messageDataAt0 === "$") {
            ...
            if (redValueSpan)
                redValueSpan.textContent = redValue.toString();
            if (greenValueSpan)
                greenValueSpan.textContent = greenValue.toString();
            if (blueValueSpan)
                blueValueSpan.textContent = blueValue.toString();
            //Y Gain
            const yGainValueSpan = document.getElementById("gbs-ygain-value");
            if (yGainValueSpan)
                yGainValueSpan.textContent = yGainValue.toString();
            //UCOS / VCOS
            const uvGainValueSpan = document.getElementById("gbs-uvgain-value");
            if (uvGainValueSpan)
                uvGainValueSpan.textContent = "U:" + uGainValue.toString() + " V:" + (uGainValue + 13).toString();
            const pad4Digits = (val) => ('0000' + val).slice(-4);
            //Move H/V
            const moveSpan = document.getElementById('gbs-move-value');
            if (moveSpan)
                moveSpan.textContent = pad4Digits(moveHValue) + "/" + pad4Digits(moveVValue);
            //Scale H/V
            const scaleSpan = document.getElementById('gbs-scale-value');
            if (scaleSpan)
                scaleSpan.textContent = pad4Digits(scaleHValue) + "/" + pad4Digits(scaleVValue);
            //Border H/V
            const borderSpan = document.getElementById('gbs-border-value');
            if (borderSpan)
                borderSpan.textContent = pad4Digits(borderHValue) + "/" + pad4Digits(borderVValue);
            //ADC Gain
...
const updateDeveloperMode = (developerMode) => {
    ...
    GBSControl.ui.developerSwitch.querySelector(".gbs-icon").innerText = developerMode ? "ON" : "OFF";
};
const updateCustomSlotFilters = (customFilters = GBSStorage.read("customSlotFilters") === true) => {
    ...
    GBSControl.ui.customSlotFilters.querySelector(".gbs-icon").innerText = customFilters ? "ON" : "OFF";
};
const GBSStorage = {
    lsObject: {},
    write(key, value) {
...
const wifiScanSSID = () => {
    ...
    fetch(`/wifi/list?${+new Date()}`)
        ...
        <td class="gbs-icon" style="opacity:${parseInt(ssid.strength, 10) / 100}">ᯤ</td>
        <td>${ssid.ssid}</td>
