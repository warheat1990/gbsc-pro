...
const Structs: StructDescriptors = {
  slots: [
    ...
    { name: "activeInputType", type: "byte", size: 1 }, // 1=RGBs, 2=RGsB, 3=VGA, 4=YPbPr, 5=SV, 6=AV (0=unset)
...
const createWebSocket = () => {
  ...
  GBSControl.ws.onmessage = (message: any) => {
    ...
    if (messageDataAt0 === "$") {
      ...
      if (advBrightnessValueSpan) advBrightnessValueSpan.textContent = advBrightnessValue.toString();
      if (advContrastValueSpan) advContrastValueSpan.textContent = advContrastValue.toString();
      if (advSaturationValueSpan) advSaturationValueSpan.textContent = advSaturationValue.toString();
      if (advHueValueSpan) advHueValueSpan.textContent = advHueValue.toString();
...
const initProButtons = () => {
  ...
  const advPicParams: { [key: string]: { param: string; max: number; default: number } } = {
  ...
  const sendAdvPicParam = (param: string, value: number) => {
    const formData = new URLSearchParams();
    formData.append(param, value.toString());
