...
void optimizeSogLevel()
{
    ...
    } else if (!rto->inputIsYpBpR && Info_sate == 0) {
...
{ // if any
    ...
    while (millis() - timeout < 450) {
        ...
        if (stable) {
            ...
            if ((currentInput == 1 && Info_sate == 0) && (SeleInputSource == S_VGA || SeleInputSource == S_RGBs)) { // RGBS or RGBHV
                ...
                if ((vsyncActive && Info_sate == 0) && (SeleInputSource == S_VGA || SeleInputSource == S_RGBs)) {
                ...
                if ((!vsyncActive && Info_sate == 0) && SeleInputSource == S_RGBs) { // then do RGBS check
                    rto->syncTypeCsync = true;
                ...
                delay(40);
            } else if (currentInput == 0 && Info_sate == 0) { // YUV
                uint16_t testCycle = 0;
...
uint8_t inputAndSyncDetect()
{
    ...
    if (syncFound == 0) {
        ...
        return 0;
    } else if (syncFound == 1 && Info_sate == 0) { // input is RGBS
        rto->inputIsYpBpR = 0;
        ...
        return 1;
    } else if (syncFound == 2 && Info_sate == 0) {
        rto->inputIsYpBpR = 1;
        rto->sourceDisconnected = false;
        rto->isInLowPowerMode = false;
        resetDebugPort();
        ...
        return 2;
    } else if (syncFound == 3 && Info_sate == 0) { // input is RGBHV
        //already applied
        rto->isInLowPowerMode = false;
        rto->inputIsYpBpR = 0;
        rto->sourceDisconnected = false;
...
void runSyncWatcher()
{
    ...
    if ((detectedVideoMode == 0 || !status16SpHsStable) && rto->videoStandardInput != 15) {
        ...
        if (rto->inputIsYpBpR && (rto->noSyncCounter == 34) && Info_sate == 0) {
