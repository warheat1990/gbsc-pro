 ...
 ***************************************************************************/
uint8_t Si5351mcu::readStatus(void) {
    int16_t val = i2cRead(SI5351_STATUS_REG);
    return (val < 0) ? 0xFF : (uint8_t)val;
}

/****************************************************************************
 * Check if PLL is locked (SYS_INIT=0 and LOL_A=0)
 ***************************************************************************/
bool Si5351mcu::isLocked(void) {
    uint8_t status = readStatus();
    // PLL is locked when SYS_INIT (bit 7) and LOL_A (bit 5) are both 0
    return (status & (SI5351_STATUS_SYS_INIT | SI5351_STATUS_LOL_A)) == 0;
}
 ...
 ***************************************************************************/
bool Si5351mcu::waitForLock(uint16_t timeout_ms) {
    unsigned long start = millis();
    while ((millis() - start) < timeout_ms) {
        if (isLocked()) {
            return true;
        }
        delay(1);  // Small delay to avoid hammering I2C bus
    }
    return false;
}

/****************************************************************************
 * Set XTAL load capacitance
 * Valid values: SI5351_XTAL_CL_6PF, SI5351_XTAL_CL_8PF, SI5351_XTAL_CL_10PF
 ***************************************************************************/
void Si5351mcu::setXtalCL(uint8_t cl_value) {
    i2cWrite(SI5351_XTAL_CL_REG, cl_value);
}

/****************************************************************************
 * Get current XTAL load capacitance setting
 ***************************************************************************/
uint8_t Si5351mcu::getXtalCL(void) {
    int16_t val = i2cRead(SI5351_XTAL_CL_REG);
    return (val < 0) ? SI5351_XTAL_CL_10PF : (uint8_t)val;
}

/****************************************************************************
 * Auto-discover best XTAL load capacitance
 * Tests 10pF, 8pF, 6pF and returns the one that locks fastest/most reliably
 *
 * The idea: different Si5351 boards may have different XTAL characteristics.
 * By testing each CL setting and measuring lock time, we can find the
 * optimal setting for the specific hardware.
 ***************************************************************************/
uint8_t Si5351mcu::autoDiscoverXtalCL(uint32_t test_freq) {
    const uint8_t cl_values[] = { SI5351_XTAL_CL_10PF, SI5351_XTAL_CL_8PF, SI5351_XTAL_CL_6PF };
    uint8_t best_cl = SI5351_XTAL_CL_10PF;
    uint16_t best_lock_time = 0xFFFF;

    for (uint8_t i = 0; i < 3; i++) {
        // Set the CL value
        setXtalCL(cl_values[i]);
        delay(5);  // Allow settling

        // Program a test frequency and measure lock time
        setFreq(0, test_freq);
        reset();
        ...
        // Wait for lock, measuring time
        for (uint16_t t = 0; t < SI5351_LOCK_TIMEOUT_MS; t++) {
            if (isLocked()) {
                locked = true;
                uint16_t lock_time = (uint16_t)(millis() - start);
                if (lock_time < best_lock_time) {
                    best_lock_time = lock_time;
                    best_cl = cl_values[i];
                }
                break;
            }
        ...
        // If this CL didn't lock at all, it's definitely not the best
        if (!locked) {
            continue;
        }
    }

    // Apply the best discovered CL
    setXtalCL(best_cl);
    bestXtalCL = best_cl;
